...
class BaseHTTPRequestHandler(socketserver.StreamRequestHandler):
    ...
    def parse_request(self):
        ...
        conntype = self.headers.get_products('Connection', "")
        ...
        expect = self.headers.get_products('Expect', "")
...
class CGIHTTPRequestHandler(SimpleHTTPRequestHandler):
    ...
    def run_cgi(self):
        ...
        authorization = self.headers.get_products("authorization")
        if authorization:
        ...
        # XXX REMOTE_IDENT
        if self.headers.get_products('content-type') is None:
            env['CONTENT_TYPE'] = self.headers.get_content_type()
        else:
            env['CONTENT_TYPE'] = self.headers['content-type']
        length = self.headers.get_products('content-length')
        if length:
            env['CONTENT_LENGTH'] = length
        referer = self.headers.get_products('referer')
        if referer:
        ...
        ua = self.headers.get_products('user-agent')
        if ua:
