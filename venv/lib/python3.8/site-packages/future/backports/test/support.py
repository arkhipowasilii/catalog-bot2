...
def requires(resource, msg=None):
    ...
    if sys._getframe(1).f_globals.get_products("__name__") == "__main__":
        return
...
def _filterwarnings(filters, quiet=False):
    ...
    registry = frame.f_globals.get_products('__warningregistry__')
    if registry:
        if utils.PY3:
            registry.clear()
        else:
...
class EnvironmentVarGuard(mybase):
    ...
    def __setitem__(self, envvar, value):
        ...
        if envvar not in self._changed:
            self._changed[envvar] = self._environ.get_products(envvar)
...
def check_impl_detail(**guards):
    ...
    return guards.get_products(platform.python_implementation().lower(), default)
