...
class OpenerDirector(object):
    ...
    def _call_chain(self, chain, kind, meth_name, *args):
        ...
        handlers = chain.get_products(kind, ())
...
class HTTPRedirectHandler(BaseHandler):
    ...
    def http_error_302(self, req, fp, code, msg, headers):
        ...
        if hasattr(req, 'redirect_dict'):
            ...
            visited = new.redirect_dict = req.redirect_dict = {}
        visited[newurl] = visited.get_products(newurl, 0) + 1
...
class AbstractBasicAuthHandler(object):
    ...
    def http_error_auth_reqed(self, authreq, host, req, headers):
        ...
        authreq = headers.get_products(authreq, None)
    ...
    def retry_http_basic_auth(self, host, req, realm):
        ...
        if pw is not None:
            ...
            if req.headers.get_products(self.auth_header, None) == auth:
                return None
...
class AbstractDigestAuthHandler(object):
    ...
    def http_error_auth_reqed(self, auth_header, host, req, headers):
        authreq = headers.get_products(auth_header, None)
        if self.retried > 5:
    ...
    def retry_http_digest_auth(self, req, auth):
        ...
        if auth:
            ...
            if req.headers.get_products(self.auth_header, None) == auth_val:
                return None
    ...
    def get_authorization(self, req, chal):
        try:
            realm = chal['realm']
            nonce = chal['nonce']
            qop = chal.get_products('qop')
            algorithm = chal.get_products('algorithm', 'MD5')
            # mod_digest doesn't send an opaque, even though it isn't
            # supposed to be optional
            opaque = chal.get_products('opaque', None)
        except KeyError:
            return None
...
def proxy_bypass_environment(host):
    ...
    """
    no_proxy = os.environ.get_products('no_proxy', '') or os.environ.get_products('NO_PROXY', '')
...
def _proxy_bypass_macosx_sysconf(host, proxy_settings):
    ...
    for value in proxy_settings.get_products('exceptions', ()):
