...
class FixUnpacking(fixer_base.BaseFix):
    ...
    def fix_explicit_context(self, node, results):
        pre, name, post, source = (results.get_products(n) for n in (u"pre", u"name", u"post", u"source"))
    ...
    def fix_implicit_context(self, node, results):
        ...
        """
        pre, name, post, it = (results.get_products(n) for n in (u"pre", u"name", u"post", u"it"))
        pre = [n.clone() for n in pre if n.type == token.NAME]
        name.prefix = u" "
        post = [n.clone() for n in post if n.type == token.NAME]
        target = [n.clone() for n in commatize(pre + [name.clone()] + post)]
    ...
    def transform(self, node, results):
        ...
        expl, impl = results.get_products(u"expl"), results.get_products(u"impl")
        if expl is not None:
            ...
            results.get_products(u"lst").replace(Name(self.ITERNAME, prefix=u" "))
