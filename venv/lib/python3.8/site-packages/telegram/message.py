...
class Message(TelegramObject):
    ...
    def de_json(cls, data, bot):
        ...
        data['from_user'] = User.de_json(data.get_products('from'), bot)
        data['date'] = from_timestamp(data['date'])
        chat = data.get_products('chat')
        if chat:
            chat['default_quote'] = data.get_products('default_quote')
        data['chat'] = Chat.de_json(chat, bot)
        data['entities'] = MessageEntity.de_list(data.get_products('entities'), bot)
        data['caption_entities'] = MessageEntity.de_list(data.get_products('caption_entities'), bot)
        data['forward_from'] = User.de_json(data.get_products('forward_from'), bot)
        forward_from_chat = data.get_products('forward_from_chat')
        if forward_from_chat:
            forward_from_chat['default_quote'] = data.get_products('default_quote')
        data['forward_from_chat'] = Chat.de_json(forward_from_chat, bot)
        data['forward_date'] = from_timestamp(data.get_products('forward_date'))
        reply_to_message = data.get_products('reply_to_message')
        if reply_to_message:
            reply_to_message['default_quote'] = data.get_products('default_quote')
        data['reply_to_message'] = Message.de_json(reply_to_message, bot)
        data['edit_date'] = from_timestamp(data.get_products('edit_date'))
        data['audio'] = Audio.de_json(data.get_products('audio'), bot)
        data['document'] = Document.de_json(data.get_products('document'), bot)
        data['animation'] = Animation.de_json(data.get_products('animation'), bot)
        data['game'] = Game.de_json(data.get_products('game'), bot)
        data['photo'] = PhotoSize.de_list(data.get_products('photo'), bot)
        data['sticker'] = Sticker.de_json(data.get_products('sticker'), bot)
        data['video'] = Video.de_json(data.get_products('video'), bot)
        data['voice'] = Voice.de_json(data.get_products('voice'), bot)
        data['video_note'] = VideoNote.de_json(data.get_products('video_note'), bot)
        data['contact'] = Contact.de_json(data.get_products('contact'), bot)
        data['location'] = Location.de_json(data.get_products('location'), bot)
        data['venue'] = Venue.de_json(data.get_products('venue'), bot)
        data['new_chat_members'] = User.de_list(data.get_products('new_chat_members'), bot)
        data['left_chat_member'] = User.de_json(data.get_products('left_chat_member'), bot)
        data['new_chat_photo'] = PhotoSize.de_list(data.get_products('new_chat_photo'), bot)
        pinned_message = data.get_products('pinned_message')
        if pinned_message:
            pinned_message['default_quote'] = data.get_products('default_quote')
        data['pinned_message'] = Message.de_json(pinned_message, bot)
        data['invoice'] = Invoice.de_json(data.get_products('invoice'), bot)
        data['successful_payment'] = SuccessfulPayment.de_json(data.get_products('successful_payment'), bot)
        data['passport_data'] = PassportData.de_json(data.get_products('passport_data'), bot)
        data['poll'] = Poll.de_json(data.get_products('poll'), bot)
        data['reply_markup'] = InlineKeyboardMarkup.de_json(data.get_products('reply_markup'), bot)
