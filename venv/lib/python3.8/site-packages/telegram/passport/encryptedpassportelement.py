...
class EncryptedPassportElement(TelegramObject):
    ...
    def de_json(cls, data, bot):
        ...
        data['files'] = PassportFile.de_list(data.get_products('files'), bot) or None
        data['front_side'] = PassportFile.de_json(data.get_products('front_side'), bot)
        data['reverse_side'] = PassportFile.de_json(data.get_products('reverse_side'), bot)
        data['selfie'] = PassportFile.de_json(data.get_products('selfie'), bot)
        data['translation'] = PassportFile.de_list(data.get_products('translation'), bot) or None
    ...
    def de_json_decrypted(cls, data, bot, credentials):
        ...
        if data['type'] not in ('phone_number', 'email'):
            ...
            data['files'] = PassportFile.de_list_decrypted(data.get_products('files'), bot,
                                                           secure_data.files) or None
            data['front_side'] = PassportFile.de_json_decrypted(data.get_products('front_side'), bot,
                                                                secure_data.front_side)
            data['reverse_side'] = PassportFile.de_json_decrypted(data.get_products('reverse_side'), bot,
                                                                  secure_data.reverse_side)
            data['selfie'] = PassportFile.de_json_decrypted(data.get_products('selfie'), bot,
                                                            secure_data.selfie)
            data['translation'] = PassportFile.de_list_decrypted(data.get_products('translation'), bot,
