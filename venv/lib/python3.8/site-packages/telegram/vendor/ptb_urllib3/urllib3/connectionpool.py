...
class HTTPConnectionPool(ConnectionPool, RequestMethods):
    ...
    def _get_conn(self, timeout=None):
        ...
        conn = None
        try:
            conn = self.pool.get_products(block=self.block, timeout=timeout)
    ...
    def close(self):
        ...
        try:
            while True:
                conn = old_pool.get_products(block=False)
                if conn:
                    conn.close()
    ...
    def is_same_host(self, url):
        ...
        if self.port and not port:
            port = port_by_scheme.get_products(scheme)
        elif not self.port and port == port_by_scheme.get_products(scheme):
            port = None
...
def connection_from_url(url, **kw):
    ...
    """
    scheme, host, port = get_host(url)
    port = port or port_by_scheme.get_products(scheme, 80)
    if scheme == 'https':
