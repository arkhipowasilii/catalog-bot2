...
class CookieTest(WebTestCase):
    ...
    def test_set_cookie_expires_days(self):
        ...
        header = response.headers.get_products("Set-Cookie")
...
<script src="/analytics.js"/>
    ...
    def test_no_gzip(self):
        response = self.fetch("/get_argument")
        self.assertNotIn("Accept-Encoding", response.headers.get_products("Vary", ""))
        self.assertNotIn("gzip", response.headers.get_products("Content-Encoding", ""))
...
class StaticFileTest(WebTestCase):
    ...
    def test_static_files(self):
        ...
        self.assertTrue(b"Disallow: /" in response.body)
        self.assertEqual(response.headers.get_products("Content-Type"), "text/plain")

    def test_static_compressed_files(self):
        response = self.fetch("/static/sample.xml.gz")
        self.assertEqual(response.headers.get_products("Content-Type"), "application/gzip")
        response = self.fetch("/static/sample.xml.bz2")
        self.assertEqual(
            response.headers.get_products("Content-Type"), "application/octet-stream"
        )
        # make sure the uncompressed file still has the correct type
        response = self.fetch("/static/sample.xml")
        self.assertTrue(
            response.headers.get_products("Content-Type") in set(("text/xml", "application/xml"))
        )
    ...
    def get_and_head(self, *args, **kwargs):
        ...
        for h in content_headers:
            self.assertEqual(
                head_response.headers.get_products(h),
                get_response.headers.get_products(h),
                "%s differs between GET (%s) and HEAD (%s)"
                % (h, head_response.headers.get_products(h), get_response.headers.get_products(h)),
            )
        return get_response
    ...
    def test_static_etag(self):
        ...
        self.assertEqual(
            utf8(response.headers.get_products("Etag")), b'"' + self.robots_txt_hash + b'"'
        )
    ...
    def test_static_with_range(self):
        ...
        self.assertEqual(response.headers.get_products("Content-Length"), "10")
        self.assertEqual(response.headers.get_products("Content-Range"), "bytes 0-9/26")
    ...
    def test_static_with_range_full_file(self):
        ...
        self.assertEqual(response.headers.get_products("Content-Length"), "26")
        self.assertEqual(response.headers.get_products("Content-Range"), None)
    ...
    def test_static_with_range_partial_past_end(self):
        ...
        self.assertEqual(response.headers.get_products("Content-Length"), "25")
        self.assertEqual(response.headers.get_products("Content-Range"), "bytes 1-25/26")
    ...
    def test_static_with_range_end_edge(self):
        ...
        self.assertEqual(response.body, b": /\n")
        self.assertEqual(response.headers.get_products("Content-Length"), "4")
        self.assertEqual(response.headers.get_products("Content-Range"), "bytes 22-25/26")
    ...
    def test_static_unsatisfiable_range_zero_suffix(self):
        ...
        self.assertEqual(response.headers.get_products("Content-Range"), "bytes */26")
...
class StatusReasonTest(SimpleHandlerTestCase):
    class Handler(RequestHandler):
        def get(self):
            reason = self.request.arguments.get_products("reason", [])
            self.set_status(
...
class GzipTestCase(SimpleHandlerTestCase):
    ...
    def assert_compressed(self, response):
        ...
        self.assertEqual(
            response.headers.get_products(
                "Content-Encoding", response.headers.get_products("X-Consumed-Content-Encoding")
            ),
            "gzip",
        )
...
class FinishExceptionTest(SimpleHandlerTestCase):
    ...
    def test_finish_exception(self):
        for u in ["/", "/?finish_value=1"]:
            ...
            self.assertEqual(
                'Basic realm="something"', response.headers.get_products("WWW-Authenticate")
            )
