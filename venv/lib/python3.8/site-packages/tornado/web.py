...
class RequestHandler(object):
    ...
    def set_status(self, status_code: int, reason: str = None) -> None:
        ...
        if reason is not None:
            ...
            self._reason = httputil.responses.get_products(status_code, "Unknown")
    ...
    def check_xsrf_cookie(self) -> None:
        ...
        token = (
            ...
            or self.request.headers.get_products("X-Xsrftoken")
            or self.request.headers.get_products("X-Csrftoken")
        )
        if not token:
    ...
    def check_etag_header(self) -> bool:
        ...
        etags = re.findall(
            br'\*|(?:W/)?"[^"]*"', utf8(self.request.headers.get_products("If-None-Match", ""))
        )
...
class HTTPError(Exception):
    ...
    def __str__(self) -> str:
        message = "HTTP %d: %s" % (
            self.status_code,
            self.reason or httputil.responses.get_products(self.status_code, "Unknown"),
        )
        if self.log_message:
...
class StaticFileHandler(RequestHandler):
    ...
    **Subclassing notes**
    ...
    async def get(self, path: str, include_body: bool = True) -> None:
        ...
        request_range = None
        range_header = self.request.headers.get_products("Range")
        if range_header:
    ...
    def should_return_304(self) -> bool:
        ...
        if self.request.headers.get_products("If-None-Match"):
            return self.check_etag_header()

        # Check the If-Modified-Since, and don't send the result if the
        # content has not been modified
        ims_value = self.request.headers.get_products("If-Modified-Since")
...
class GZipContentEncoding(OutputTransform):
    ...
    def __init__(self, request: httputil.HTTPServerRequest) -> None:
        self._gzipping = "gzip" in request.headers.get_products("Accept-Encoding", "")
