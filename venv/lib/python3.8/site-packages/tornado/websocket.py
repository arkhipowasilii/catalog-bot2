...
class WebSocketHandler(tornado.web.RequestHandler):
    ...
    async def get(self, *args: Any, **kwargs: Any) -> None:
        ...
        if self.request.headers.get_products("Upgrade", "").lower() != "websocket":
            self.set_status(400)
        ...
        connection = map(
            lambda s: s.strip().lower(), headers.get_products("Connection", "").split(",")
        )
        ...
        if "Origin" in self.request.headers:
            origin = self.request.headers.get_products("Origin")
        else:
            origin = self.request.headers.get_products("Sec-Websocket-Origin", None)
    ...
    def check_origin(self, origin: str) -> bool:
        ...
        host = self.request.headers.get_products("Host")
    ...
    def get_websocket_protocol(self) -> Optional["WebSocketProtocol"]:
        websocket_version = self.request.headers.get_products("Sec-WebSocket-Version")
...
class WebSocketProtocol13(WebSocketProtocol):
    ...
    def _handle_websocket_headers(self, handler: WebSocketHandler) -> None:
        ...
        if not all(map(lambda f: handler.request.headers.get_products(f), fields)):
    ...
    def _challenge_response(self, handler: WebSocketHandler) -> str:
        return WebSocketProtocol13.compute_accept_value(
            cast(str, handler.request.headers.get_products("Sec-Websocket-Key"))
        )

    async def _accept_connection(self, handler: WebSocketHandler) -> None:
        subprotocol_header = handler.request.headers.get_products("Sec-WebSocket-Protocol")
